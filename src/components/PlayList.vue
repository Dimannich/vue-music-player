<template>
  <div class="root">
    <slick-list
      @mouseout="handleOut"
      helperClass="draggable-item--moved"
      :useDragHandle="true"
      v-model="items"
      class="mt-5"
      lockAxis="y"
      @input="handleMove(false)"
      @sort-start="handleMove(true)"
    >
      <slick-item v-for="(item, index) in items" :index="index" :key="index">
        <div class="draggable-item d-flex">
          <div
            :class="{'draggable-item__handle--hover': !disableMouseOver}"
            @mouseout="handleOut"
            @mouseover="handleOver"
            v-handle
            class="draggable-item__handle h-100 d-flex justify-content-center align-items-center"
          >
